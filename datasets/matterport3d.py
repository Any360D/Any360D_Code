...
def expand_list(short_list, repeat_len):
    short_len = len(short_list)
    
    # Calculate the number of times short_list needs to be repeated and the remaining elements
    repeat_count = repeat_len // short_len
    remainder = repeat_len % short_len
    
    # Expand the short list
    expanded_list = short_list * repeat_count + short_list[:remainder]
...
class Matterport3D(data.Dataset):
    ...
    def __init__(self, root_dir, list_file, height=504, width=1008, color_augmentation=True,
        ...
        if self.is_training:
            self.rgb_depth_list = expand_list(self.rgb_depth_list, repeat)
    ...
    def __getitem__(self, idx):
        ...
        # cv2.imwrite('label_gt.png', depth)
